...
public class BackendRegistry {
    ...
    public BackendRegistry(WsServerEndpoint websocket) {
        ...
        if(withFakeBackend) {
            ...
            var identifier = new BackendIdentifier(DemoBackendResource.DEMO_BACKEND_INFO.id);
            Log.debugv("registering fake backend {0} with endpoint {1}",identifier,fakeBackendEndpoint);
            this.registry.put(identifier,new BackendProxy(DemoBackendResource.DEMO_BACKEND_INFO,PoiRemoteService.createRestClient(fakeBackendEndpoint)));
        }
    }
